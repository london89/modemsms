...
//  $modemlist=SQLSelect("SELECT * FROM modemsms_devices WHERE CHECK_NEXT<=NOW()");
  if(method_exists($this, 'sendnotification')) {
      ...
      if ((time()-$last_timestamp > 1*30*24*60*60) || !$last_timestamp) { // если с последнего оповещения прошло больше месяца
        ...
        if ($sms_count['count'] > 2000) $this->sendnotification('В Вашей базе хранится уже '.$sms_count['count'].' смс сообщений. Уверены, что это нормально?:) Не ставьте галочку "ничего не делать" в настройках смс модема, если не понимаете о чем речь. Либо отмечайте как прочитанное, либо удаляйте их с модема.', 'danger');
      }
  }
